...
def build_dfg_figure(
    ...
    # node_text = [wrap_label(a, max_chars=14) for a in nodes_sorted]
    # node_text = [wrap_label(a, max_chars=12, max_lines=3) for a in nodes_sorted]
    NODE_SIZE = 64      # 圆直径(px)；你可以改 60~72
    FONT_SIZE = 9       # 字体(px)

    # 经验系数：每个字符大约占 0.6~0.65 个 font_size 的宽度
    ...
    fig.add_trace(
        go.Scatter(
            ...
            customdata=nodes_sorted,  # hover 仍显示完整名字
