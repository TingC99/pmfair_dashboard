...
def build_dfg_figure(
    ...
    for _, r in df.iterrows():
        ...
        head = f"<b>{node_code_map.get(s,'?')} → {node_code_map.get(d,'?')}</b><br>{s} → {d}<br>"
    ...
    # --- nodes (codes inside circles)
    node_codes = [node_code_map.get(a, "?") for a in nodes_sorted]
    fig.add_trace(
        go.Scatter(
...
with central_pane:
    ...
    if view_mode != "Global Path Patterns":
        ...
        if fig is None:
            ...
            # ---- Render DFG + handle badge click via Streamlit native selection ----
            ...
            # Parse selection (robust across Streamlit versions)
            sel = None
            ...
            if pts and not badge_df.empty:
                ...
                if curve is not None and curve < len(fig.data):
                    is_badge_trace = (getattr(fig.data[curve], "meta", None) == "badge")
                    if is_badge_trace:
