...
def build_dfg_figure(
    ...
    def _wrap(s: str, n: int = 18) -> str:
        ...
        if cut <= 0:
            cut = n
        return s[:cut] + "<br>" + s[cut:].lstrip()

    node_text = [_wrap(a, 18) for a in nodes_sorted]
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker=dict(size=34, color="rgba(235,235,235,1.0)", line=dict(width=1, color="rgba(80,80,80,0.6)")),
                    ...
                    if curve is not None and 0 <= int(curve) < len(fig.data):
                        if getattr(fig.data[int(curve)], "meta", None) == "badge":
                            ...
                            if idx is not None:
                                ...
                                if 0 <= idx < len(badge_df):
                                    st.session_state.selected_badge_edge = badge_df.iloc[idx].to_dict()
# (Details are shown as a pinned tooltip next to the clicked badge on the graph.)
            with st.expander("Pinned unfairness details (debug)", expanded=False):
                if st.session_state.get("selected_badge_edge"):
                    st.json(st.session_state.selected_badge_edge)
