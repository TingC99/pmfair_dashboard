...
def build_sankey_figure(edge_stats: pd.DataFrame,
                        nodes_sorted,
                        x_norm,
                        y_norm,
                        min_support: int,
    ...
    """
    Returns:
      fig, badge_df (unfair edges drawn as markers)
    """
    if edge_stats.empty:
    ...
    node_index = {a: i for i, a in enumerate(nodes_sorted)}
    node_codes = [node_code_map[a] for a in nodes_sorted]

    # keep only edges whose nodes exist
    df = df[df["src"].isin(node_index) & df["dst"].isin(node_index)].copy()
    if df.empty:
        return None, pd.DataFrame()

    sources = df["src"].map(node_index).tolist()
    targets = df["dst"].map(node_index).tolist()
    values = df["case_support_all"].astype(float).tolist()

    # link coloring: unfair = red
    # link_colors = ["rgba(220,0,0,0.55)" if u else "rgba(120,120,120,0.25)" for u in df["is_unfair"].tolist()]
    link_colors = []
    for _, r in df.iterrows():
        if r["is_unfair"]:
            alpha = 0.60 if r["is_focus"] else 0.08
            link_colors.append(f"rgba(220,0,0,{alpha})")
        else:
            alpha = 0.30 if r["is_focus"] else 0.06
            link_colors.append(f"rgba(120,120,120,{alpha})")

    # hover info
    edge_text = []
    for s, d in zip(df["src"].tolist(), df["dst"].tolist()):
        edge_text.append(f"{node_code_map[s]} → {node_code_map[d]}<br>{s} → {d}")
    if measure == "Δpp":
        customdata = np.stack([
            np.array(edge_text),
            df["p_tgt"].to_numpy(),
            df["p_ref"].to_numpy(),
            df["delta_pp"].to_numpy(),
            df["support_tgt"].to_numpy(),
            df["support_ref"].to_numpy(),
            df["n_tgt"].to_numpy(),
            df["n_ref"].to_numpy(),
        ...
        hovertemplate = (
            "<b>%{customdata[0]}</b><br>"
            "Case frequency: %{value:.0f}<br>"
            "p(target=" + str(tgt_label) + "): %{customdata[1]:.3f} "
            "(%{customdata[4]:.0f}/%{customdata[6]:.0f})<br>"
            "p(ref=" + str(ref_label) + "): %{customdata[2]:.3f} "
            "(%{customdata[5]:.0f}/%{customdata[7]:.0f})<br>"
            "Δpp: <b>%{customdata[3]:+.1f}pp</b><br>"
        ...
        hovertemplate = (
            ...
            "p(ref=" + str(ref_label) + "): %{customdata[2]:.3f} "
            "(%{customdata[5]:.0f}/%{customdata[7]:.0f})<br>"
            "Ratio: <b>%{customdata[3]:.2f}×</b><br>"
            "Unfair: %{customdata[8]}<extra></extra>"
        )
    node_centers_x = [x_norm[a] for a in nodes_sorted]
    node_centers_y = [y_norm[a] for a in nodes_sorted]
    x=[x_norm[a] for a in nodes_sorted],
    y=[y_norm[a] for a in nodes_sorted],


    sankey = go.Sankey(
        arrangement="fixed",
        # domain=dict(x=[0, 1], y=[0.04, 0.96]),
        node=dict(
            label=node_codes,
            align="justify",   # ✅ Node Alignment：让横向更均匀，确实有用
            pad=6,
            thickness=24,
    ...
    # --- badge markers for unfair links ---
    if highlight_activity:
    ...
    # 给每个 src 的不公平边做排序，便于错开
        ...
        for src, g in badge_df.groupby("src"):
            ...
            dsts_sorted = sorted(
                dsts,
                key=lambda d: (x_norm.get(d, 0.5), y_norm.get(d, 0.5), str(d))
            )
            out_rank[src] = {dst: i for i, dst in enumerate(dsts_sorted)}
            out_count[src] = len(dsts_sorted)

        bx, by, btext, bcustom, bhover = [], [], [], [], []
        alpha = 0.30        # ✅ 徽章放在 src->dst 的 30% 处
        spread = 0.025      # ✅ 同一 src 多个徽章的错开幅度

        for _, r in badge_df.iterrows():
            src, dst = r["src"], r["dst"]
            sx, sy = x_norm.get(src, 0.5), y_norm.get(src, 0.5)
            tx, ty = x_norm.get(dst, 0.5), y_norm.get(dst, 0.5)

            vx, vy = (tx - sx), (ty - sy)

            # 如果是回边/loop（目标不在右侧），用 fallback：放在源节点右侧
            if vx <= 1e-6:
                x = min(sx + 0.045, 0.98)
                y = sy
            else:
                ...
                nx, ny = (-vy / norm, vx / norm)  # unit normal

                k = out_count.get(src, 1)
                i = out_rank.get(src, {}).get(dst, 0)
                offset = (i - (k - 1) / 2) * spread
            ...
            btext.append("!")
            bcustom.append(f"{src} → {dst}")

            # 徽章 hover 简单点就行（真正的数值看 link hover）
            bhover.append("Unfair edge (see link hover for details)")
...
with central_pane:
    ...
    st.caption("Sankey: edge width = case frequency. Unfair edges are red with a badge (!) you can click.")
    ...
    if view_mode != "Global Path Patterns":
        ...
        legend_rows = [(act_to_code[a], a) for a in nodes_sorted if a in act_to_code]
        ...
        # node layout + sankey
        nodes_sorted, x_norm, y_norm = compute_node_layout(events_f)
        codes = make_codes(len(nodes_sorted))
        node_code_map = {a: c for a, c in zip(nodes_sorted, codes)}

        fig, badge_df = build_sankey_figure(
        ...
        if fig is None:
            ...
            if HAS_PLOTLY_EVENTS:
                # curveNumber=0 is sankey, curveNumber=1 is badge scatter (if exists)
                ...
                if clicked:
                    ev = clicked[0]
                    # Only handle badge clicks (scatter trace)
                    if ev.get("curveNumber", -1) == 1:
                        idx = ev.get("pointIndex", None)
                        if idx is not None and idx < len(badge_df):
                            st.session_state.selected_badge_edge = badge_df.iloc[int(idx)].to_dict()
            else:
                st.plotly_chart(fig, use_container_width=True)
                st.warning("要实现“点击徽章显示详情”，请安装：pip install streamlit-plotly-events")
            ...
            # legend_items = [f"**{node_code_map[a]}** = {a}" for a in nodes_sorted]
            # cols_legend = st.columns(3)
            # for i, item in enumerate(legend_items):
            #     cols_legend[i % 3].markdown(item)
            st.markdown("#### Node Legend (current view)")
            cols = st.columns(3)
            for i, (c, a) in enumerate(legend_rows):
                cols[i % 3].markdown(f"**{c}** = {a}")
